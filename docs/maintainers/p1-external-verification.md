...
Run this procedure only after the protected `v0.4.3` workflow has published
...
The result must identify 0.4.3 on `experimental`, retain 0.3.2 on `latest`, and
...
invocations and post that exact statement as a comment on issue #210. The
...
node scripts/verify-p1-external-results.mjs \
  ...
  --release-url https://github.com/codeacme17/launchrally/releases/tag/v0.4.3 \
...
`experimental-0.4.3-p1-evidence.md`: host and exact version, public workflow
