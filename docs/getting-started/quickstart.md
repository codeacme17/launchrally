...
Already using an initialized Experimental `0.4.1` project? Use the separate
[exact 0.4.1-to-0.4.2 Project Toolchain migration](../maintainers/p1-migration-notes.md#project-toolchain-migration-041-to-042).
...
After confirmation, Human Init prints a concise completion with the outcome, Manifest action and source Report, Project Toolchain version, aggregate applied-change count, and the exact shell-safe `rally --version --json --cwd <repository-root>` command derived from the Launcher entry. Run that mandatory read-only authority probe before Plan, Handoff, or Verify, and continue only when it reports both `authority.state: "ready"` and `authority.source: "project_toolchain"`. Detailed applied paths remain available through the unchanged structured Agent/JSON Init result without flooding the default Human completion.
