...
Detected route files are unclassified candidates, not confirmed public Journeys. Human Mode requires an explicit `public`, `user`, `staff`, `signed_token`, or exclude decision for each detected or separately supplied route. Protected classification is available for exact non-root static GET paths with alphabetic application-specific segments; compound slug and numeric identifier segments remain excluded. Syntax alone cannot prove that an arbitrary alphabetic word is not a personal identifier, so never submit one as a path segment. LaunchRally never infers authorization from route names. Dynamic or parameterized candidates must be excluded unless the builder separately supplies a concrete safe path. Protocol-relative or absolute targets, traversal, backslashes, queries, fragments, percent encoding, credentials, and out-of-origin targets remain outside the protected boundary. The final Audit Brief shows the resulting access class and anonymous/authenticated status expectations before it requests the separate public and authenticated-read permissions for every exact path.
