...
```bash
codex plugin marketplace add codeacme17/launchrally --ref v0.2.0
...
Publishing is performed only by the repository's release workflow on a matching `v0.2.0` tag. npm Trusted Publishing supplies short-lived OIDC credentials and provenance; no long-lived npm token is used.
