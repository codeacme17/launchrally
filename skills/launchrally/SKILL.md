...
3. If `rally` is absent, explain that CLI installation and Plugin installation are separate, present exactly `npm install --global @launchrally/cli@0.2.2` and the structured verification command, then stop before Audit and wait for the user. Do not run the install, automatically substitute the separate exact-version npm-exec trial/CI entry, or change npm prefixes or shell profiles.
