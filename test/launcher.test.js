...
test("every repository operation resolves to the project Engine", async () => {
  const repository = await projectWithEngine([
    "const operations = new Set([\"audit\", \"architect\", \"architecture-package\", \"init\", \"plan\", \"providers\", \"verify\"]);",
    "process.stdout.write(process.argv.slice(2).find((value) => operations.has(value)));",
    "",
  ].join("\n"));
  const operations = ["audit", "architect", "architecture-package", "init", "plan", "providers", "verify"];
  const selected = [];
