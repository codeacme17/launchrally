...
test("the README terminal visual is accessible, responsive, and fixture-derived", async () => {
  ...
  assert.match(
    readme,
    /<picture>[\s\S]*?<source media="\(prefers-color-scheme: dark\)" srcset="docs\/assets\/launchrally-terminal-dark\.svg">[\s\S]*?<img src="docs\/assets\/launchrally-terminal-light\.svg" alt="LaunchRally terminal showing an Inconclusive audit with verification gaps and the next init command" width="760">[\s\S]*?<\/picture>/u,
  );
  ...
  for (const theme of ["light", "dark"]) {
    ...
    assert.match(svg, /viewBox="0 0 520 440"/u);
