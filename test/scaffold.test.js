...
test("audit excludes Architecture history without hiding protected LaunchRally inputs", async () => {
  ...
  const initial = await runAudit(fixture, "0.4.3");
  const confirmation = await runAudit(fixture, "0.4.3", {
  ...
  const permission = await runAudit(fixture, "0.4.3", {
    resume_token: confirmation.interaction.resume_token,
    confirmation: "confirm",
  });
  const result = await runAudit(fixture, "0.4.3", {
