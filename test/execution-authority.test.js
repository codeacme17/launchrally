...
async function writeConfirmedProject(repository, version = "0.3.0") {
...
async function materializeEngine(repository, version = "0.3.0") {
...
test("process cwd discovers a ready project Engine from a repository subdirectory", async () => {
  ...
  await writeConfirmedProject(repository, "0.3.0");
  await materializeEngine(repository, "0.3.0");
  ...
  assert.equal(authority.engine.version, "0.3.0");
...
test("transaction recovery and inconsistent installed identity fail closed", async () => {
  ...
  await writeConfirmedProject(recoveringRepository, "0.3.0");
  ...
  await writeConfirmedProject(inconsistentRepository, "0.3.0");
...
test("symlinked authority files and path-escaping toolchains fail closed", async () => {
  ...
  await writeConfirmedProject(descriptorRepository, "0.3.0");
...
test("version failure states are structured, non-zero, and omit cli_version", async () => {
  ...
  await writeConfirmedProject(restoreRepository, "0.3.0");
  let restoreFailure;
  ...
  await writeConfirmedProject(migrationRepository, "0.3.0");
  await writeFile(
