...
Save a completed Agent Mode Audit and pass it to `rally init --report <path>`. Initialization displays exact before/after contents, digests, and diffs for every authoritative proposed change. Only `--confirm confirm` applies the previewed `.launchrally` Manifest and ignore rules, exact CLI devDependency, lockfile update, and complete source Report history. Report bundles become visible only after their canonical Record, View, and Evidence Index are staged completely; allowlisted safe Evidence is stored by SHA-256 address. Init does not write the non-authoritative current-Report cache. Declining, stale previews, and dependency-planning failures write nothing; interrupted partial writes are rolled back or recovered from the ignored ownership-bound transaction journal on the next `init`. Concurrent Init/recovery operations for the same repository fail safely instead of interleaving adoption. LaunchRally never stages or commits these project-owned files and never automatically prunes historical Reports or Evidence.
