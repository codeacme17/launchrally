...
- the exact `@launchrally/cli` devDependency in `.launchrally/toolchain/package.json`;
- the isolated `.launchrally/toolchain/package-lock.json`.
...
The toolchain is always npm-based and independent of the application's ecosystem or package manager. Init attempts exact resolution from npm's offline cache first. On a cache miss it returns `needs_permission` and discloses `@launchrally/cli`, its exact version, `https://registry.npmjs.org`, and the lifecycle-script-disabled npm command. Resume with an explicit `npm_registry_read` decision. Denial writes nothing; approval permits only that registry read and then returns the exact file preview. Application manifests and lockfiles are never changed.
...
Ordinary Init never replaces an existing canonical Manifest. Its Human and JSON results identify the existing Manifest-bound source Report, the supplied Report being adopted into immutable history, and the typed `--rebind` next action. If a corrected current Audit Report should replace project-owned release intent, invoke `rally init --json --cwd <repository-root> --report <corrected-audit-report-json> --rebind`. Present the old and new source Report identities, the immutable history changes, and the separate `release_intent_replacement` Manifest diff. Pass `--confirm confirm` only after the builder explicitly accepts that replacement; `decline`, abandonment, a stale preview, invalid or incomplete Report data, or inconsistent Evidence changes nothing.

A confirmed rebind atomically rebuilds every Report-derived Manifest field and `execution.source_report_id` from the corrected Report. It preserves the exact Project Toolchain pin and all existing immutable Reports and Evidence. Retain the corrected Report as the new `<manifest-source-report-json>` and use it for the subsequent required full Verify.
