...
2. Require `@launchrally/cli@0.2.1` and check a project-pinned CLI first.
3. Before any journey command, invoke that executable with `--version --json` and require `contract: "launchrally.dev/cli/v2"`, `operation: "version"`, `status: "completed"`, and `cli_version: "0.2.1"`; stop on any mismatch.
