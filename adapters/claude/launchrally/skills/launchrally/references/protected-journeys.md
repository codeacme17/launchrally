...
Static route discovery identifies a possible path; it does not establish whether anonymous, user, staff, or signed-token access is intended. Human Mode discloses the complete detected set, then requires the builder to retain an explicit subset and exclude the remainder, or exclude the complete set. Classify only retained routes as `public`, `user`, `staff`, `signed_token`, or excluded, then review the complete Audit Brief before confirmation. A public classification authorizes an anonymous GET and asserts a 200-299 response. A protected classification records the disclosed anonymous denial statuses separately from the expected authenticated response and never grants access to authentication material.

Protected classification accepts application-specific route names only when the target is an exact, non-root, slash-delimited static path and the method is GET. Each bounded canonical segment uses lowercase ASCII letters and digits, with single hyphens or underscores only between them. Dynamic placeholders and obvious opaque numeric, hexadecimal, or UUID segment shapes remain excluded. Syntax cannot establish authorization, an authentication class, or whether a name contains personal data, so classify only an exact concrete path that is appropriate to disclose. Protocol-relative or absolute targets, traversal or dot segments, empty or trailing separators, backslashes, query strings, fragments, percent encoding, credentials, and out-of-origin targets remain outside the boundary. Human Mode reports the rejected constraint when protected classification is unavailable. When discovery reports a dynamic route, exclude that candidate and explicitly supply a separate exact concrete path if one is appropriate; approval covers only the disclosed same-origin path and no adjacent route.
