...
const NEGATIVE_LOCAL_EVIDENCE = Object.freeze({
  accepted_kinds: ["local_observation"],
  minimum_items: 1,
  provenance_required: true,
});

const LAYERED_CHECK_METADATA = Object.freeze({
  "web.baseline.package-manifest": ["local_implementation", ["runtime_execution", "ci_cd"]],
  "web.baseline.lockfile": ["local_implementation", ["ci_cd"]],
  "web.baseline.runtime-inputs": ["local_implementation", ["secrets_configuration"]],
  "web.baseline.build-command": ["local_implementation", ["ci_cd"]],
  "web.public.availability": ["operational_delivery", ["runtime_execution"]],
  "web.public.transport-security": ["operational_delivery", ["dns_tls"]],
  "web.baseline.data-state": ["local_implementation", ["application_data"]],
  "web.baseline.observability": ["provider_configuration", ["operational_observability"]],
  "web.public.core-journeys": ["downstream_outcome", ["runtime_execution"]],
});

function providerCapabilityIds(roles) {
  const capabilities = new Set();
  for (const role of roles) {
    if (["data", "database"].includes(role)) capabilities.add("application_data");
    else if (["storage", "object_storage"].includes(role)) capabilities.add("object_storage");
    else if (["identity", "authentication"].includes(role)) {
      capabilities.add("identity_authentication");
    } else if (["email", "communication"].includes(role)) {
      capabilities.add("communication_delivery");
    } else if (role === "observability") capabilities.add("operational_observability");
    else if (["hosting", "deployment", "runtime"].includes(role)) {
      capabilities.add("runtime_execution");
    }
  }
  return [...capabilities];
}
...
function declaration({
  ...
  failure_evidence_requirement = pass_evidence_requirement,
  verification_rules,
  severity,
  gate,
  freshness,
  ...
  const [defaultLayer, defaultCapabilities] = LAYERED_CHECK_METADATA[check_id] ?? [];
  return Object.freeze({
    check_id,
    check_version: 1,
    risk_domain,
    check_layer: check_layer ?? defaultLayer,
    capability_ids: capability_ids ?? defaultCapabilities,
    permission_id,
    applicability: {
...
const CHECKS = Object.freeze([
  {
    declaration: declaration({
      ...
      pass_evidence_requirement: FILE_EVIDENCE,
      verification_rules: [
  ...
  },
  {
    declaration: declaration({
      ...
      pass_evidence_requirement: FILE_EVIDENCE,
      failure_evidence_requirement: NEGATIVE_LOCAL_EVIDENCE,
      verification_rules: [
    ...
    verify(project) {
      ...
      if (project.safe_scan?.coverage?.root_lockfiles?.complete !== true) {
        return unverified(
          "uncovered_scope",
          "Root lockfile collection was incomplete, so absence cannot be treated as a negative finding.",
        );
      }
      return failed(
        ...
        {
          state: "complete",
          required_targets: ["repository:root-lockfiles"],
          observed_targets: ["repository:root-lockfiles"],
        },
      );
    },
  },
  ...
  },
  {
    ...
    verify(project) {
      ...
      return failed(
        ...
        {
          state: "complete",
          required_targets: ["repository:package.json#scripts.build"],
          observed_targets: ["repository:package.json#scripts.build"],
        },
      );
    },
  },
...
function localObservation(project, target, outcome) {
  return {
    ...
    collection: {
      root: project.root,
      complete: project.safe_scan?.errors?.length === 0,
      exclusions: structuredClone(project.safe_scan?.exclusions ?? {}),
    },
    provenance: {
      collector: project.safe_scan?.policy_version ?? "local_safe_scan/v1",
      exact_target: target,
    },
  };
}
...
function providerCheckDeclaration(request) {
  ...
  return declaration({
    ...
    check_layer: "provider_configuration",
    capability_ids: providerCapabilityIds(request.roles),
    ...
    pass_evidence_requirement: MACHINE_EVIDENCE,
    failure_evidence_requirement: MACHINE_EVIDENCE,
    verification_rules: [
...
function executeProviderCheck(request, providerResult, environment) {
  ...
  return {
    ...
    result: {
      ...resultBase(declared, applicability, environment),
      ...result,
    },
  };
}
...
function resultBase(declared, applicability, environment) {
  return {
    check_id: declared.check_id,
    check_version: declared.check_version,
    risk_domain: declared.risk_domain,
    check_layer: declared.check_layer,
    capability_ids: structuredClone(declared.capability_ids),
    environment,
    priority: "p0",
...
function executeCheck(
  ...
  const base = resultBase(
    check.declaration,
    applicability,
    auditBrief.intended_environment.value,
  );
  if (
...
export function executeWebBaseline({
  ...
  const providerChecks = providerRequests.map((request) =>
    executeProviderCheck(request, provider_result, audit_brief.intended_environment.value),
  );
