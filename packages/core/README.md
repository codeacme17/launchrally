...
`referenceIntegrationPacks`, `createReferenceCoverageMatrix`, and `referenceExecutorDescriptors` expose the reviewed Phase 1 reference roster. The eight Packs cover five representative product shapes with two materially different managed implementations per family plus retained, custom, self-hosted, and Unknown fallbacks. Each managed implementation carries a distinct deterministic synthetic input that normalizes into the same Provider-neutral observation; the observation remains unverified and cannot create Machine Evidence or change assurance. Provider commands, API versions, and provenance remain inside each Pack. Existing read Adapters and active-test recipes are accepted only for their exact reviewed family and implementation, so unsupported depth remains generic or a transparent Gap. The two core-catalog Executors are exact-version Codex CLI and Claude Code descriptors with local-source authority only; neither grants installation, login, Provider, deployment, secret, or production-data authority.

`planActiveVerification`, `approveActiveVerification`, and `reviewActiveVerificationObservation` form the separate Active Verification interface. Reviewed, versioned recipes cover minimized synthetic webhook, user, checkout, email, object-upload, and CI-dispatch journeys. Each request binds an exact environment, versioned Integration Contract, Executor mode, Task Graph, Handoff Package, synthetic fixture, correlation ID, effects, observation window, timeout, cleanup strategy, and independent approval. Approval and observation rederive those bindings against current Catalog and Executor reviews. Production-like environments remain default-denied; only an explicitly production-safe recipe can be separately approved. Results distinguish transport acceptance from business outcomes and normalize timeout, late success, duplicates, successful or exhausted retries, ordering, replay, eventual consistency, failure visibility, and cleanup. Observation normalization requires an external Executor attestation verifier and persists only its content digest and secret-free provenance. Only a successful observed outcome with successful cleanup can produce privacy-safe Evidence; a downstream Check also revalidates the exact approved plan and attestation. Denial, unsupported inputs, and every non-qualifying result remain a Verification Gap.
...
```sh
npm install @launchrally/core@0.4.3
```

```js
import { runAudit } from "@launchrally/core";

const interaction = await runAudit(process.cwd(), "0.4.3");
