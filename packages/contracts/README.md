...
Integration Contracts use one idempotency vocabulary: `required`, `not_required`, `not_applicable`, or `unknown`. A required contract may declare its normalized deduplication identifier separately as `idempotency_key`. Readers continue to accept persisted `required_by_<key>` and `deduplicate_by_<key>` values for migration; newly created contracts canonicalize those forms to `required` plus `idempotency_key`, while other free-form values fail validation.
...
```sh
npm install @launchrally/contracts@0.4.3
```
